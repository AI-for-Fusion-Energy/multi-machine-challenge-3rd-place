...
def Train_and_Validate(net,optimizer,scheduler,criterion,max_norm, train_shotlist,validate_shotlist,max_epoch,break_loss):
    ...
    for epoch in range(max_epoch):
        Train_Loss_Epoch,T_accu = Train_n1n(net,optimizer,scheduler,criterion,train_shotlist,epoch)
        Validate_Loss_Epoch,V_accu = Validate_n1n(net,criterion,validate_shotlist,epoch)
...
def Train_n1n(net,optimizer,scheduler,criterion,train_shotlist,epoch):
    Train_Loss_Epoch = []
...
def Validate_n1n(net,criterion,validate_shotlist,epoch):
    Validate_Loss_Epoch = []
...
def Sample_Generator(filelist):
    ...
    for file in filelist:
        ...
        #         print(a[i])
        a = np.nan_to_num(np.array(a[:-60]))
        if np.size(a) <0.8*Norm_Length:
